...
1.732 x 1.732 mm 300-grain (v_smoke) mesh.

Replaces the coupled temperature-displacement formulation used by
build_inp_smoke.py with a *Static analysis in which sigma_c is hard-coded
per deck via the Serebrinsky polynomial. This removes the USDFLD coupling,
the temperature DOF on bulk and cohesive elements, and all *Initial
Conditions blocks; bulk elements revert from CPE3T to CPE3 and cohesive
elements stay COH2D4.

Physics parameters (K_COH = 1e6 MPa/mm, VISC = 5e-2, sigma_c(0) = 1000 MPa,
DISP = 0.433 mm for 25% strain on a 1.732 mm gauge) follow the upstream
build_inp_smoke.py calibration. C0 sweep is
[0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0] wppm.
...
E, NU = 207000.0, 0.29
RHO   = 8.47e-9
# Note: conductivity/specific_heat dropped (no thermal solve in *Static).
...
# Used only to compute theta_init -> per-deck sigma_c; not written into the
# deck (no thermal solve to receive it).
N_A   = 6.022e23
M_H   = 1.008e-6
...
# 1c. Sanity-check sequential node numbering (informational, not used for IC).
...
# Assembly-level elsets for step-output requests. Dropped ALL_NODES nset
# (was for *Initial Conditions, no longer needed).
assembly_inserts = (
...
# ============================================================================
# Step 3  Section Controls block (positioned before *Part as in smoke build)
# ============================================================================
...
def make_materials(sigma_c_deck):
    """Materials block with uniform per-deck sigma_c. No field dependency,
    no Depvar, no conductivity / specific_heat (no thermal solve)."""
    return f"""**
** MATERIALS
**
...
for c0 in C0_VALUES:
    ...
    # Theta from Langmuir-McLean (floored at 0 for c0=0 case).
...
"""
    ...
    heading = (f'*Heading\n'
               f'HEDE Inconel 600 STATIC -- C0 = {c0} wppm '
...
print(f'\n[done] {len(written)} static decks written to {JOBS_DIR}')
